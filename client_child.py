...
from utils import (
    flatten_dict,
    transfer_time_consistency_check,
    purchase_consistency_check,
    is_chinese,
...
)  # Telegram 机器人的 API Token 老鲸鱼
TELEGRAM_CHAT_ID = config.get(
    "TELEGRAM", "TELEGRAM_CHAT_ID"
)  # 你的 Telegram 用户或群组 ID  老鲸鱼
TELEGRAM_BOT_TOKEN_BAOJI = config.get(
    "TELEGRAM", "TELEGRAM_BOT_TOKEN_BAOJI"
)  # Telegram 机器人的 API Token   暴击的
TELEGRAM_CHAT_ID_BAOJI = config.get(
    "TELEGRAM", "TELEGRAM_CHAT_ID_BAOJI"
)  # 你的 Telegram 用户或群组 ID 暴击的
TELEGRAM_BOT_TOKEN_15DAYS = config.get(
    "TELEGRAM", "TELEGRAM_BOT_TOKEN_15DAYS"
)  # Telegram 机器人的 API Token   15day老钱包
TELEGRAM_CHAT_ID_15DAYS = config.get(
    "TELEGRAM", "TELEGRAM_CHAT_ID_15DAYS"
)  # 你的 Telegram 用户或群组 ID 15day老钱包
TELEGRAM_BOT_TOKEN_ZHUANZHANG = config.get(
    "TELEGRAM", "TELEGRAM_BOT_TOKEN_ZHUANZHANG"
)  # Telegram 机器人的 API Token   老钱包转账
TELEGRAM_CHAT_ID_ZHUANZHANG = config.get(
    "TELEGRAM", "TELEGRAM_CHAT_ID_ZHUANZHANG"
)  # 你的 Telegram 用户或群组 ID 老钱包转账
...
# 2025.2.4日更新 检测持有者需要用到
MINT_HOLDINGS_DATA = "mint_holdings_data:"  # mint持有者列表 缓存10秒
ADDRESS_HOLDINGS_ALERT_DATA = (
    "address_holdings_alert_data:"  # 用户最近活跃 ##分析感叹号数量
)
...
async def fetch_config(server_id=SERVER_ID):
    ...
    global TYPE1_SETTINGS_TRANSACTION_ENABLED, TYPE1_SETTINGS_PURCHASE_AMOUNT_RANGE, TYPE1_SETTINGS_DAY_INTERVAL, TYPE1_SETTINGS_MARKET_CAP_LIMIT, TYPE1_SETTINGS_SOL_LIMIT, TYPE1_SETTINGS_TOKEN_BALANCE
    global TYPE2_SETTINGS_TRANSACTION_ENABLED, TYPE2_SETTINGS_PURCHASE_AMOUNT_RANGE, TYPE2_SETTINGS_DAY_INTERVAL, TYPE2_SETTINGS_MARKET_CAP_LIMIT, TYPE2_SETTINGS_PROFIT_PER_TOKEN, TYPE2_SETTINGS_PROFIT_RATE_PER_TOKEN
    global TYPE3_SETTINGS_TRANSACTION_ENABLED, TYPE3_SETTINGS_DAY_INTERVAL, TYPE3_SETTINGS_PURCHASE_AMOUNT_RANGE
    global TYPE4_SETTINGS_TRANSACTION_ENABLED, TYPE4_SETTINGS_DAY_INTERVAL, TYPE4_SETTINGS_PARENT_WALLET_TRANSFER_SOL, TYPE4_SETTINGS_TOKEN_QUANTITY, TYPE4_SETTINGS_TOKEN_BALANCE, TYPE4_SETTINGS_PURCHASE_AMOUNT_RANGE, TYPE4_SETTINGS_PARENT_WALLET_TRANSACTION_RANGE
    global ALLOWED_TRAN_TYPES, TRANSACTION_ADDRESS_GROUP
    global HOLDER_CHECK_ENABLED, HOLDER_CHECK_SCOPE, HOLDER_CHECK_SORT
    global LARGE_TXN_CHECK_ENABLED, LARGE_TXN_THRESHOLD
    global BUY_CONSISTENCY_CHECK_ENABLED, BUY_CONSISTENCY_TIME_RANGE, BUY_CONSISTENCY_SINGLE_TXN_AMOUNT, BUY_CONSISTENCY_ALLOWED_TIMES
    global TRANSFER_CONSISTENCY_CHECK_ENABLED, TRANSFER_CONSISTENCY_TIME_RANGE, TRANSFER_CONSISTENCY_ALLOWED_TIMES
    global SIMILARITY_CHECK_ENABLED, SIMILARITY_PHISHING_WALLET_CHECK_ENABLED, SIMILARITY_PHISHING_WALLET_COUNT, SIMILARITY_INSIDER_TRADING_CHECK_ENABLED, SIMILARITY_INSIDER_TRADING_COUNT, SIMILARITY_SAME_SOURCE_WALLET_CHECK_ENABLED, SIMILARITY_SAME_SOURCE_WALLET_COUNT, SIMILARITY_DEV_TEAM_CHECK_ENABLED, SIMILARITY_DEV_TEAM_COUNT, SIMILARITY_TOTAL_CHECK_ENABLED, SIMILARITY_TOTAL_COUNT
    # 全局设置
    ...
    # 持有者检测总设置
    HOLDER_CHECK_ENABLED = None  # 总开关
    HOLDER_CHECK_SCOPE = None  # 检查范围
    HOLDER_CHECK_SORT = None  # 列表排序
    # 持有者检测 大额检测
    LARGE_TXN_CHECK_ENABLED = None  # 开关
    LARGE_TXN_THRESHOLD = None  # 金额上限
    # 持有者检测 购买一致性检测
    BUY_CONSISTENCY_CHECK_ENABLED = None  # 开关
    BUY_CONSISTENCY_TIME_RANGE = None  # 时间范围 设置1是一秒内，设置2就是2秒内
    BUY_CONSISTENCY_SINGLE_TXN_AMOUNT = None  # 金额范围 [min,max]
    BUY_CONSISTENCY_ALLOWED_TIMES = None  # 允许单数
    # 持有者检测 转账时间一致性检测
    TRANSFER_CONSISTENCY_CHECK_ENABLED = None  # 开关
    TRANSFER_CONSISTENCY_TIME_RANGE = None  # 时间范围 设置1是一秒内，设置2就是2秒内
    TRANSFER_CONSISTENCY_ALLOWED_TIMES = None  # 允许单数
    # 持有者检测 相似性检测
    SIMILARITY_CHECK_ENABLED = None  # 开关
    SIMILARITY_PHISHING_WALLET_CHECK_ENABLED = None  # 钓鱼钱包筛选勾选
    SIMILARITY_PHISHING_WALLET_COUNT = None  # 允许数量
    SIMILARITY_INSIDER_TRADING_CHECK_ENABLED = None  # 老鼠仓勾选
    SIMILARITY_INSIDER_TRADING_COUNT = None  # 允许数量
    SIMILARITY_SAME_SOURCE_WALLET_CHECK_ENABLED = None  # 通钱包来源 勾选
    SIMILARITY_SAME_SOURCE_WALLET_COUNT = None  # 允许数量
    SIMILARITY_DEV_TEAM_CHECK_ENABLED = None  # dev团队 勾选
    SIMILARITY_DEV_TEAM_COUNT = None  # 允许数量
    SIMILARITY_TOTAL_CHECK_ENABLED = None  # 总计 勾选
    SIMILARITY_TOTAL_COUNT = None  # 允许数量

    # 类型一设置
    TYPE1_SETTINGS_TRANSACTION_ENABLED = None  # 交易开关
    TYPE1_SETTINGS_PURCHASE_AMOUNT_RANGE = None  # 单独设置购买金额 是个范围 [min,max]
    ...
    TYPE2_SETTINGS_PURCHASE_AMOUNT_RANGE = None  # 单独设置购买
    ...
    TYPE3_SETTINGS_PURCHASE_AMOUNT_RANGE = None  # 单独设置购买金额
    TYPE3_SETTINGS_DAY_INTERVAL = None  # 单独设置间隔时间
    # 类型四设置
    TYPE4_SETTINGS_TRANSACTION_ENABLED = None  # 交易开关
    TYPE4_SETTINGS_PURCHASE_AMOUNT_RANGE = None  # 单独设置交易金额
    ...
    if response.status_code == 200:
        ...
        for i in range(1, 5):
            if not config.get(f"type{i}_settings_purchase_amount_range"):
                config[f"type{i}_settings_purchase_amount_range"] = config[
        ...
        config["transaction_address_group"] = (
            url.replace(" ", "").split("\n") if url else []
        )
        # 订阅周期秒化
        config["subscription_cycle"] = config["subscription_cycle"] * 60
        # 设定单子的最少时间过期
        config["redis_ex_time"] = min(
            config["type1_settings_day_interval"],
            config["type2_settings_day_interval"],
            config["type3_settings_day_interval"],
            config["type4_settings_day_interval"],
        )
        # 设置单子的最小排除单笔购买
        config["min_purchase_amount"] = min(
            config["type1_settings_purchase_amount_range"][0],
            config["type2_settings_purchase_amount_range"][0],
            config["type3_settings_purchase_amount_range"][0],
...
async def cleanup_subscriptions():
    ...
    while True:
        ...
        logging.error(
            f"----数据处理队列 {market_cap_sol_height_update_mq_list.qsize()} 条----"
        )
                ...
                while True:
                    ...
                    try:
                        ...
                        if "txType" in message:
                            ...
                            # 当前时间的时间戳格式
                            message["create_time_stamp"] = int(
                                time.time() * 1000
                            )  # 毫秒级
                            # 计算代币美元单价
                            message["sol_price_usd"] = sol_price["price"]
                            if txType == "create":
                                await subscribed_new_mq_list.put(
                                    message
                                )  # 识别订单创建
                                ...
                                if mint in subscriptions:
                                    ...
                                    if (
                                        ...
                                        > subscriptions[mint]["market_cap_sol_height"]
                                    ):
                                        subscriptions[mint].update(
                                            {
                                                "market_cap_sol_height": message[
                                                    "marketCapSol"
                                                ],
                                        ...
                                        # 存过后台的，直接刷新市值
                                        r.set(
                                            f"{MINT_NEED_UPDATE_MAKET_CAP}{mint}",
                                            json.dumps(subscriptions[mint]),
                                            xx=True,
                                            ex=7200,
                                        )
                                # 扫描符合要求的订单
                                if message["solAmount"] >= MIN_PURCHASE_AMOUNT:
                                    lock_acquired = r.set(
                                        f"{TXHASH_SUBSCRBED}{message['signature']}",
                                        "原子锁1秒",
                                        nx=True,
                                        ex=1,
                                    )  # 锁5秒自动过期
        ...
        if rank is not None and rank == 0:  # 排名最前面，开始消费
            ...
            while True:
                ...
                if product_data:
                    ...
                    logging.info(
                        f"代币 {message['mint']} 用户 {message['traderPublicKey']} {message['signature']}  交易金额:{message['solAmount']}"
                    )
                    ...
                    if time.time() - last_task_time >= max_wait_time:
                        logging.warning(
                            f"客户端 {CLIENT_ID} 已等待超过 {max_wait_time} 秒，强制更新分数并返回队列"
                        )
                        rank_score = r.incr(f"{CLIENT}client_counter")  # 强制更新分数
                        r.zadd(f"{CLIENT}{CLIENT_MQ_LIST}", {CLIENT_ID: rank_score})
                        # 更新客户端的最后分数更新时间
                        r.hset(
                            f"{CLIENT}{CLIENT_ID}_last_update_time", "time", time.time()
                        )
...
async def check_inactive_clients():
    ...
    while True:
        ...
        for client_id in clients:
            ...
            if rank_score:
                # 获取客户端最后更新时间
                last_update_time = r.hget(
                    f"{CLIENT}{client_id}_last_update_time", "time"
                )
                if last_update_time:
                    last_update_time = float(last_update_time)
                    if current_time - last_update_time >= SCORE_UPDATE_THRESHOLD:
                        logging.warning(
                            f"客户端 {client_id} 已超过 {SCORE_UPDATE_THRESHOLD} 秒未更新分数，移除该客户端"
                        )
...
def transactions_message_no_list(item):
    ...
    r = redis_client()
    check = r.set(
        f"{ADDRESS_EXPIRY}{item['traderPublicKey']}",
        "周期排除",
        nx=True,
        ex=int(REDIS_EX_TIME * 86400),
    )
    if check:
        ...
        transactions_data = fetch_user_transactions(
            start_time, now.timestamp(), item
        )  # 获取近365天内的20条交易记录
        ...
        future_mint_holdings = executor.submit(fetch_mint_holdings, item)

        # 等待任务完成
        dev_data = future_dev.result()
        alert_data = future_alert.result()
        wallet_data = future_wallet_info.result()
        mint_holdings_data = future_mint_holdings.result()
        ...
        # 开始检查持仓者数据
        if HOLDER_CHECK_ENABLED:  # 总开关
            if LARGE_TXN_CHECK_ENABLED and large_transaction_check(
                mint=item["mint"],
                data=mint_holdings_data,
                max_amount=LARGE_TXN_THRESHOLD,
                sol_usd=sol_price["price"],
                logging=logging,
            ):  # 大额检测开关
                return
            if BUY_CONSISTENCY_CHECK_ENABLED and purchase_consistency_check(
                mint=item["mint"],
                data=mint_holdings_data,
                amount_range=BUY_CONSISTENCY_SINGLE_TXN_AMOUNT,
                time_range=BUY_CONSISTENCY_TIME_RANGE,
                allowed_times=BUY_CONSISTENCY_ALLOWED_TIMES,
                sol_usd=sol_price["price"],
                logging=logging,
            ):  # 购买一致性
                return
            if TRANSFER_CONSISTENCY_CHECK_ENABLED and transfer_time_consistency_check(
                mint=item["mint"],
                data=mint_holdings_data,
                time_range=TRANSFER_CONSISTENCY_TIME_RANGE,
                allowed_times=TRANSFER_CONSISTENCY_ALLOWED_TIMES,
                logging=logging,
            ):  # 转账时间一致性
                return
            if SIMILARITY_CHECK_ENABLED and similarity_check(
                mint=item["mint"],
                data=mint_holdings_data,
                phishing_wallet_enabled=SIMILARITY_PHISHING_WALLET_CHECK_ENABLED,
                phishing_wallet_count=SIMILARITY_PHISHING_WALLET_COUNT,
                insider_trading_edabled=SIMILARITY_INSIDER_TRADING_CHECK_ENABLED,
                insider_trading_count=SIMILARITY_INSIDER_TRADING_COUNT,
                source_wallet_enabled=SIMILARITY_SAME_SOURCE_WALLET_CHECK_ENABLED,
                source_wallet_count=SIMILARITY_SAME_SOURCE_WALLET_COUNT,
                dev_team_enabled=SIMILARITY_DEV_TEAM_CHECK_ENABLED,
                dev_team_count=SIMILARITY_DEV_TEAM_COUNT,
                total_enabled=SIMILARITY_TOTAL_CHECK_ENABLED,
                total_count=SIMILARITY_TOTAL_COUNT,
                logging=logging,
            ):  # 相似性检测
                return
        # 4种type都需要用到的数据
        ...
        item["mint_create_time_utc"] = item["subscriptions"][
            "create_time_utc"
        ]  # 代币创建时间

        if (
            item["solAmount"] >= TYPE1_SETTINGS_PURCHASE_AMOUNT_RANGE[0]
            and item["solAmount"] < TYPE1_SETTINGS_PURCHASE_AMOUNT_RANGE[1]
        ):
            if not calculate_time_diff(
                item, 1, transactions_data, TYPE1_SETTINGS_DAY_INTERVAL
            ):
                executor.submit(type1, item)  # 老鲸鱼
        if (
            item["solAmount"] >= TYPE2_SETTINGS_PURCHASE_AMOUNT_RANGE[0]
            and item["solAmount"] < TYPE2_SETTINGS_PURCHASE_AMOUNT_RANGE[1]
        ):
            if not calculate_time_diff(
                item, 2, transactions_data, TYPE2_SETTINGS_DAY_INTERVAL
            ):
                executor.submit(type2, item)  # 老鲸鱼
        if (
            item["solAmount"] >= TYPE3_SETTINGS_PURCHASE_AMOUNT_RANGE[0]
            and item["solAmount"] < TYPE3_SETTINGS_PURCHASE_AMOUNT_RANGE[1]
        ):
            if not calculate_time_diff(
                item, 3, transactions_data, TYPE3_SETTINGS_DAY_INTERVAL
            ):
                executor.submit(type3, item)
        if (
            item["solAmount"] >= TYPE4_SETTINGS_PURCHASE_AMOUNT_RANGE[0]
            and item["solAmount"] < TYPE4_SETTINGS_PURCHASE_AMOUNT_RANGE[1]
        ):
            if not calculate_time_diff(
                item, 4, transactions_data, TYPE4_SETTINGS_DAY_INTERVAL
            ):
...
def type4(item):
    ...
    start_time = int(
        (
            now - timedelta(days=TYPE4_SETTINGS_PARENT_WALLET_TRANSACTION_RANGE)
        ).timestamp()
    )  # 获取近72小时内的转账记录
    transfer_data = fetch_user_transfer(
        start_time, now.timestamp(), item["traderPublicKey"]
    )
    father_address = None
    for value in transfer_data:
        sol_amount = value["amount"] / (10 ** value["token_decimals"])
        if (
            sol_amount >= TYPE4_SETTINGS_PARENT_WALLET_TRANSFER_SOL
            and value["from_address"] not in exchange_wallets
        ):  # 2个以上转账 并且不是交易所地址
    ...
    sol = data.get("sol")
    if (
        len(tokens) < TYPE4_SETTINGS_TOKEN_QUANTITY
        or total_balance < TYPE4_SETTINGS_TOKEN_BALANCE
    ):
        logging.error(
            f"代币 {item['mint']} 老钱包 {father_address} 的数值 => 代币数量 {len(tokens)} 设定值 {TYPE4_SETTINGS_TOKEN_QUANTITY}  代币余额 {total_balance} 设定值 {TYPE4_SETTINGS_TOKEN_BALANCE} 不满足 type4"
        )
        return
    ...
    send_telegram_notification(
        tg_message_html_5(item),
        [TELEGRAM_BOT_TOKEN_ZHUANZHANG, TELEGRAM_CHAT_ID_ZHUANZHANG],
        f"用户 {item['traderPublicKey']} 转账老钱包",
        item,
    )
...
def type3(item):
    ...
    """
    logging.info(
        f"代币 {item['mint']} 发现了{TYPE3_SETTINGS_DAY_INTERVAL}天钱包 {item['traderPublicKey']} type3"
    )
    mint = item["mint"]
    ...
    if wallet_count >= 2:
        ...
        send_telegram_notification(
            tg_message_html_4(item),
            [TELEGRAM_BOT_TOKEN_15DAYS, TELEGRAM_CHAT_ID_15DAYS],
            f"代币 {mint} 15天钱包",
            item,
        )
...
def type1(item):
    ...
    try:
        # 市值检测
        if (
            item["market_cap"] < TYPE1_SETTINGS_MARKET_CAP_LIMIT
        ):  # 老鲸鱼暴击的条件 小于设定的市值时
            logging.error(
                f"代币 {mint} 的市值 {item['market_cap']} 设定 {TYPE1_SETTINGS_MARKET_CAP_LIMIT} 不满足 type1"
            )
            return
        # tokens 余额检测
        ...
        if total_balance < TYPE1_SETTINGS_TOKEN_BALANCE:
            logging.error(
                f"代币 {mint} 用户 {traderPublicKey} 的代币余额 {total_balance} 设定 {TYPE1_SETTINGS_TOKEN_BALANCE} 不满足 type1"
            )
            return
        ...
        if save_success_to_redis(mint=mint, type=1, address=traderPublicKey):
            ...
            item["total_balance"] = total_balance
            send_telegram_notification(
                tg_message_html_1(item),
                [TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID],
                f"用户 {traderPublicKey} {item['title']}",
                item,
            )
...
def type2(item):
    ...
    try:
        # 市值检测
        if (
            item["market_cap"] < TYPE2_SETTINGS_MARKET_CAP_LIMIT
        ):  # 老鲸鱼暴击的条件 小于设定的市值时
            logging.error(
                f"代币 {mint} 的市值 {item['marketCapSol']} 设定 {TYPE2_SETTINGS_MARKET_CAP_LIMIT} 不满足 type2"
            )
            return
        ...
        hold_data = data
        if (
            float(hold_data["realized_profit"]) < TYPE2_SETTINGS_PROFIT_PER_TOKEN
            or float(hold_data["realized_pnl"]) < TYPE2_SETTINGS_PROFIT_RATE_PER_TOKEN
        ):
            logging.error(
                f"代币 {mint} 用户 {traderPublicKey} 单笔最大盈利(已结算) {hold_data['realized_profit']} usdt 小于设定值 {TYPE2_SETTINGS_PROFIT_PER_TOKEN} 盈利率为{(float(hold_data['realized_pnl']) * 100):.2f}% 小于设定值 {(float(TYPE2_SETTINGS_PROFIT_RATE_PER_TOKEN) * 100):.2f}% type2"
            )
            return
        # 检测 并发送到交易端
        ...
        if save_success_to_redis(mint=mint, type=2, address=traderPublicKey):
            ...
            send_telegram_notification(
                tg_message_html_3(hold_data),
                [TELEGRAM_BOT_TOKEN_BAOJI, TELEGRAM_CHAT_ID_BAOJI],
                f"用户 {traderPublicKey} {hold_data['title']}",
                item,
            )
...
def calculate_time_diff(item, type, transactions_data, day_interval):
    ...
    if time_diff < (day_interval * 86400):
        logging.error(
            f"用户 {item['traderPublicKey']} {item['mint']} 请求type-{type} 时间不满足 {time_diff/86400} 设定值 {day_interval}"
        )
        return True
    return False
...
def fetch_wallet_info(item):
    ...
    if data := redis_client().get(f"{WALLET_INFOS}{address}"):
        ...
        proxies = {"https": proxy_expired.get("proxy")}
        res = gmgn_api.getWalletInfo(
            walletAddress=address, period="7d", proxies=proxies
        )
    ...
    if PROFIT_7D and realized_profit_7d is not None and realized_profit_7d < PROFIT_7D:
        logging.error(
            f"用户 {address} 7天内结算盈利额度 {realized_profit_7d} 设置 {PROFIT_7D}"
        )
        return True
...
def fetch_user_wallet_holdings(address):
    ...
    if data:
        ...
        proxies = {"https": proxy_expired.get("proxy")}
        res = gmgn_api.getWalletHoldings(
            walletAddress=address,
            params="limit=1&orderby=realized_profit&direction=desc&showsmall=true&sellout=true&tx30d=true",
            proxies=proxies,
        )
        if res.status_code == 200:
            ...
            if holdings:
                ...
                redis_client().set(
                    f"{ADDRESS_HOLDINGS_DATA}{address}",
                    json.dumps(holdings_data),
                    ex=int(86400 * REDIS_EX_TIME),
                )
            else:
...
def fetch_mint_dev(item):
    ...
    if res:
        ...
        proxies = {"https": proxy_expired.get("proxy")}
        res = gmgn_api.getTokenTrades(
            token=mint,
            params="limit=20&event=buy&maker=&tag[]=dev_team",
            proxies=proxies,
        )
        if res.status_code == 200:
    ...
    if len(data) > 1:  # 有小号
        if sols >= DEV_TEAM_SOL_WITH_SUB:
            logging.error(
                f"代币 {mint} dev团队存在小号 持仓 {sols} sol超过 {DEV_TEAM_SOL_WITH_SUB}"
            )
            return True
    else:  # 没小号
        if sols >= DEV_TEAM_SOL_WITHOUT_SUB:
            logging.error(
                f"代币 {mint} dev团队不存在小号 持仓 {sols} sol超过 {DEV_TEAM_SOL_WITHOUT_SUB}"
            )
            return True
...
def fetch_user_tokens(address):
    ...
    portfolio_calculator = PortfolioValueCalculatorJUP(
        balances_api_key=HELIUS_API_KEY, account_address=address
    )
    data = {
        "total_balance": portfolio_calculator.calculate_total_value(),
        "sol": portfolio_calculator.get_sol(),
        "tokens": portfolio_calculator.get_tokens(),
    }
...
def fetch_user_account_sol(address):
    ...
    response = requests.get(
        f"https://pro-api.solscan.io/v2.0/account/detail?address={address}",
        headers=headers,
    )
    if response.status_code == 200:
...
def send_to_trader(item, type):
    ...
    # 看看redis是否已经发送到交易端了
    status = redis_client().set(
        f"{MINT_SUCCESS}{mint}", symbol, nx=True, ex=int(86400 * REDIS_EX_TIME)
    )
    if not status:
        # 失败原因放进去
    ...
    # 开始通知交易端 多线程等待返回
    futures = [
        executor.submit(call_trade, mint, url, type)
        for url in TRANSACTION_ADDRESS_GROUP
    ]
    # 等待并获取所有结果
    msg = ""
    flag = False
...
def call_trade(mint, call_back_url, type):
    try:
        ...
        if response.status_code == 200:
            ...
            logging.error(
                f"代币 {mint} 调用交易端地址 {call_back_url} 失败 代号 {type} statusCode:{response.status_code}"
            )
            return {
                "success": False,
                "msg": f"调用交易端地址 {call_back_url} 失败 statusCode: {response.status_code}",
            }
    except Exception as e:
        logging.error(f"代币 {mint} 调用交易端地址 {call_back_url} 失败: {str(e)}")
        return {
            "success": False,
            "msg": f"调用交易端地址 {call_back_url} 失败: {str(e)}",
        }
...
def fetch_user_wallet_holdings_show_alert(item):
    ...
    res = gmgn_api.getWalletHoldings(
        walletAddress=address,
        params="limit=50&orderby=last_active_timestamp&direction=desc&showsmall=true&sellout=true&tx30d=true",
        proxies=proxies,
    )
    ...
    holdings_data = [
        h
        for h in res.json().get("data", {}).get("holdings", [])
        if h["token"]["address"] != mint
    ]
    ...
    proportion = (
        sum(1 for h in holdings_data if h.get("token", {}).get("is_show_alert"))
        / total_count
    )
    ...
    redis_client().set(
        f"{ADDRESS_HOLDINGS_ALERT_DATA}{address}",
        proportion,
        ex=int(86400 * REDIS_EX_TIME),
    )
    # 判断是否超出黑名单阈值
...
# 获取持有者
def fetch_mint_holdings(item):
    mint = item["mint"]
    data = redis_client().get(f"{MINT_HOLDINGS_DATA}{mint}")
    if data:
        return json.loads(data)
    else:
        proxies = {"https": proxy_expired.get("proxy")}
        res = gmgn_api.getTokenTopHolders(
            token=mint,
            params=f"limit=20&cost=20&orderby={HOLDER_CHECK_SORT}&direction=desc",
            proxies=proxies,
        )
        ...
        if res.status_code == 200:
            ...
            data = [
                ...
                and not value["name"] == "Raydium Authority V4"
                and not value["native_transfer"]["name"]
            ]  # 过滤掉资金流动池地址和交易所地址 name 中出现这个就是已经发射了
        else:
            data = []
            logging.error(f"代币 {mint} 持有者失败 {res.text}")
        if data:
            logging.info(f"代币 {mint} 持有者已缓存")
            redis_client().set(f"{MINT_HOLDINGS_DATA}{mint}", json.dumps(data), ex=10)
        else:
            logging.error(f"代币 {mint} 持有者数据为空不能缓存")
    return data
...
def fetch_black_wallets():
    ...
    if response.status_code == 200:
        ...
        exchange_wallets = [
            entry["walletAddress"] for entry in data if entry["type"] == 1
        ]
...
def save_transaction(item):
    ...
    redis_client().set(
        f"{MINT_NEED_UPDATE_MAKET_CAP}{item['mint']}",
        json.dumps(item["subscriptions"]),
        ex=7200,
    )
...
async def fetch_maket_data(mint):
    ...
    res = gmgn_api.getKline(
        token=mint,
        type="mcapkline",
        params=f"resolution=1s&from={int(start_time.timestamp())}&to={int(now.timestamp())}",
        proxies=proxies,
    )
    if res.status_code == 200:
        data = res.json()["data"]
        if data:
            ...
            logging.info(
                f"代币 {mint} 最高市值 {max_high_data['high']} 时间差 {int(time_diff /1000)} 秒"
            )
...
def analyze_transaction_records(transactions_data, item, count):
    ...
    for (
        ...
        transactions_data
    ):  # 有几种情况 1.用户今天只交易了一条没有以往的数据 first_time有值 last_time 是none  sum <= 10 2.用户今日数据超标 first有值 last_time 是none sum > 10 3.今日用户没有交易 但是有以往的数据 first_time 是none last_time 是 有值的 sum是0
        if (
            value["block_time"] - today > 0
        ):  # 区块链时间减去今天0点的时间大于0 代表今天之内交易的
            routers = value.get(
                "routers", {}
            )  # token1 的值 1.sol原生代币 2.sol的原始币 3.usdc
            if (
                routers
                and "token1" in routers
                and (
            ...
            last_time = value[
                "block_time"
            ]  # 当区块链时间有一个是今天以外的时间，将这个对象取出并结束循环
            break
    ...
    logging.info(
        f"用户 {item['traderPublicKey']} 今日买入 {sum}笔 今日第一笔和之前最后一笔交易时间差为 {time_diff} 天"
    )
...
async def main():
    ...
    await asyncio.gather(
        subscribed_new_task,
        ws_task,
        fair_consumption_task,
        check_inactive_clients_task,
        market_cap_sol_height_task,
        redis_task,
        cleanup_task,
    )
