...
# 持有者检测-转账时间一致性检测
def transfer_time_consistency_check(mint, data, time_range, allowed_times, logging):
    """
    持有者检测-转账时间一致性检测
    data 是数据
    allowed_times 是允许次数
    ...
    # 定义允许的最大时间差（2秒）
    if not data:
        logging.error(f"代币 {mint} 转账时间一致性检测 数据是空")
        return True
    ...
    for item in grouped_data:
        if len(item) > allowed_times:
            logging.error(f"代币 {mint} 转账时间一致性检测 => 发现转账时间 {time_range} 的持仓者有 { len(item)} 名设定{allowed_times}")
            return True
    logging.info(f"代币 {mint} 转账时间一致性检测 =>  未发现持仓者在检测范围 {time_range} 秒内 转账")
    return False


# 购买一致性检测
def purchase_consistency_check(
    mint, data, amount_range, time_range, allowed_times, sol_usd, logging
):
    """
    持有者检测-购买一致性检测
    data 是数据
    ...
    # 定义允许的最大时间差（2秒）
    if not data:
        logging.error(f"代币 {mint} 购买一致性检测 数据是空")
        return True
    max_time_diff = time_range - 1
    ...
    for item in grouped_data:
        if len(item) > allowed_times:
            logging.error(f"代币 {mint} 一致性检测 => 发现购买时间在 {time_range} 的持仓者有 { len(item)} 名设定{allowed_times}")
            return True
    logging.info(f"代币 {mint} 一致性检测 => 未发现持仓者在检测范围 {time_range} 秒内 购买")
    return False


# 大额交易检测
def large_transaction_check(mint, data, max_amount, sol_usd, logging):
    """
    data 是数据
    max_amount 是检测上限金额
    sol_usd 是sol的usd市价
    """
    if not data:
        logging.error(f"代币 {mint} 大额检测 数据是空")
        return True
    for item in data:
        cost_sol = item["cost"] / sol_usd
        if cost_sol > max_amount:
            logging.error(f"代币 {mint} 大额检测 => {cost_sol}sol 设定{max_amount}sol")
            return True
    logging.info(f"代币 {mint} 大额检测 => 未发现超出设定值的持仓者 设定{max_amount}sol")
    return False
...
def similarity_check(
    mint,
    data,
    phishing_wallet_enabled,
    phishing_wallet_count,
    insider_trading_edabled,
    insider_trading_count,
    source_wallet_enabled,
    source_wallet_count,
    dev_team_enabled,
    dev_team_count,
    total_enabled,
    ...
    """
    if not data:
        logging.error(f"代币 {mint} 相似度检测 数据是空")
        return True
    ...
    logging.info(f"代币 {mint} 相似度检测")
    logging.info(f"总数 => {len(sum_data)}")
    logging.info(f"钓鱼钱包 => {len(transfer_in)}")
    logging.info(f"老鼠仓 => {len(rat_trader)}")
    logging.info(f"dev_team => {len(dev_team)}")
    logging.info(f"同源钱包 => {len(grouped_data)} 组")
    if total_enabled and len(sum_data) > total_count:  # 总开关开启时
        logging.error(f"代币 {mint} 相似度检测 => 总检测 => 总和条数{len(sum_data)} 设定 {total_count}")
        return True
    else:  # 计算单独开关
        if (
            phishing_wallet_enabled and len(transfer_in) > phishing_wallet_count
        ):  # 钓鱼钱包
            logging.error(f"代币 {mint} 相似度检测 => 钓鱼钱包 => 条数{len(transfer_in)} 设定 {phishing_wallet_count}")
            return True
        if (
            insider_trading_edabled and len(rat_trader) > insider_trading_count
        ):  # 老鼠仓
            logging.error(f"代币 {mint} 相似度检测 => 老鼠仓 => 条数{len(rat_trader)} 设定 {insider_trading_count}")
            return True
        if source_wallet_enabled:  # 同一个转账来源
            for from_address, items in grouped_data.items():
                logging.info(f"同源钱包 => {len(items)} 个")
        ...
        if dev_team_enabled and len(dev_team) > dev_team_count:
            logging.error(f"代币 {mint} 相似度检测 => dev => 条数{len(dev_team)} 设定 {dev_team_count}")
            return True
    logging.info(f"代币 {mint} 相似度检测 => 未发现超标的持仓者")
    return False
